...
export function encryptCredentials(
  ...
  if (!passphrase) {
    throw new Error("A passphrase is required to encrypt IMAP credentials");
  }
...
export function decryptCredentials(
  ...
  if (!passphrase) {
    throw new Error("A passphrase is required to decrypt IMAP credentials. Set MAILBOX_MCP_PASSPHRASE or pass it directly.");
  }
  ...
  return JSON.parse(decrypted.toString("utf-8")) as ImapCredentials;
}
